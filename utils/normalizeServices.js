...
module.exports = async function normalizeServices(rows, clientId) {
  ...
  for (const r of rows || []) {
    ...
    const baseAmountFromAdditional =
      isAdditionalService
        ? Number(
            r.amount ??
              r.fixedCharges ??
    ...
    const travelDate = r.travelDate || r.serviceStartDate || undefined;
    ...
    items.push({
      ...
      serviceDueDate: r.serviceDueDate ? new Date(r.serviceDueDate) : undefined,
